...
class SHT21:
    ...
    # _TRIGGER_TEMPERATURE_NO_HOLD = br'\xF3'
    _TRIGGER_TEMPERATURE_NO_HOLD = b'\xF3'
    # _TRIGGER_TEMPERATURE_NO_HOLD = br'\xF3'
    #_TRIGGER_TEMPERATURE_NO_HOLD = int('0x3a',16)
    # _TRIGGER_HUMIDITY_NO_HOLD = 0xF5
    _TRIGGER_HUMIDITY_NO_HOLD = b'\xF5'
    #_TRIGGER_HUMIDITY_NO_HOLD = 245
    ...
    def __init__(self, device_number=1):
        ...
        # fcntl.ioctl(self.i2c, self.I2C_SLAVE_FORCE, 0x40)
        # self.i2c.write(chr(self._SOFTRESET))
        self.i2c.write(self._SOFTRESET)
        time.sleep(0.050)
    ...
    def read_temperature(self):    
        ...
        self.i2c.write(self._TRIGGER_TEMPERATURE_NO_HOLD)
        time.sleep(self._TEMPERATURE_WAIT_TIME)
        data = self.i2c.read(3)
        # print(self._TRIGGER_TEMPERATURE_NO_HOLD)
        # print(data)
        
        # if self._calculate_checksum(data, 2) == ord(data[2]):
        if self._calculate_checksum(data, 2) == ord( bytes([data[2]]) ):
    ...
    def read_humidity(self):    
        ...
        self.i2c.write(self._TRIGGER_HUMIDITY_NO_HOLD)
    ...
    def _get_temperature_from_buffer(data):
        ...
        unadjusted = (ord(   bytes([data[0]])   ) << 8) + ord(   bytes([data[1]])   )
...
def getTemperature():
    try:
        with SHT21(0) as sht21:
            ...
            print("Temperature is :"+str(temp)+" Celcius")
            return temp
    except:
...
def getHumidity():
    try:
        with SHT21(0) as sht21:
            temp = sht21.read_humidity()
            print("Humidity is :"+str(temp)+" %")
            return temp
    except:
